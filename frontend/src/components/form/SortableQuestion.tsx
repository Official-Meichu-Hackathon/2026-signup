...
function SortableRow({ option, index }: { option: string; index: number }) {
  ...
  return (
    <div className="flex items-center gap-3">
      <div className="rounded-lg px-1.5 py-1 text-[0.5625rem] font-bold whitespace-nowrap text-white md:px-3 md:py-2 md:text-xl">
      ...
      >
        ...
        <span
          className={`text-darkblue text-[0.5625rem] md:text-xl ${
...
export default function SortableQuestion({
  ...
  return (
    <div className="py-2 md:py-8">
          ...
          <DragOverlay>
            {activeId ? (
              <div className="group border-darkblue flex items-center rounded-md border-[0.1875rem] bg-white p-2 shadow-2xl md:p-3">
                <DragHandle />
                <span className="text-darkblue text-[0.5625rem] md:text-xl">
                  {activeId}
                </span>
              </div>
            ) : null}
      ...
      <div className="mx-4 mt-2 text-[0.5rem] text-white/70 md:text-xs">
        💡 拖拉項目來調整順序
      </div>
    </div>
  )
